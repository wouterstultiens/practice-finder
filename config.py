"""
Configuration for all dental practices to monitor.

Each dictionary in the PRACTICES list must contain:
- name: Human-readable name of the practice.
- url: The full URL of the vacancy page.
- selector: The CSS selector for the content area to monitor.
- get_html:
    - True: Monitor the raw HTML of the selected element.
    - False: Monitor only the extracted text content.
- ignore_selectors (optional): A list of CSS selectors within the main selector
  to remove before processing. Useful for ignoring dynamic content like ads,
  timestamps, or cookie banners.
"""
